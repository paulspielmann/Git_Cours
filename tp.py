...
def main():
    ...
    mesures(1000, gen_tab_int(1000, 1, 1000), inclus_rapide, 100)
    # mesures(100000)
    # mesures(100000)
    test_inclus()
...
# tries : nb d'essais
# Retourne le temps d'exec moyen sur 'tries' essais de la fonction func lorsque
# sont passe un tab aleatoire de taille n et val
def mesure(n, val, mini, maxi, func, tries):
    i = 0
    res = 0

    while i < tries:
        # Generate random array size n range [mini...maxi]
...
# Prends en param un tableau de taille n, contenant a chaque indice
# i une taille de tableau a generer, passe a la fonction mesure()
# cette taille et les autres params
# Retourne un tableau res ou res[i] => resultat de mesure() pour taille[i]
...
# Appelle plusieurs fois mesures multiples et print le resultat
# de maniere plus ou moins degueulasse
def mesures(n, val, func, tries):
    tailles = [100, 1000, 10000, 20000, 50000]
    result = mesure_multiples(tailles, val, 1, n, func, tries)
